...
if __name__ == '__main__':
    ...
    model.summary()
    visualize_nn(model, "descripyopn", figsize=(10, 8))
    # compile the neural network
    model.compile(optimizer='adam', loss='binary_crossentropy',
                  metrics=['accuracy', F1])

    # train/validation
    training = model.fit(x=X, y=y, batch_size=32, epochs=50, shuffle=True, verbose=0, validation_split=0.3)
    ...
    explainer_shap(model,
                   X_names=['temperature', 'greencolor', 'randomparam', 'randomparam', 'randomparam', 'randomparam', 'randomparam', 'randomparam', 'randomparam', 'randomparam'],
                   X_instance=X[i],
                   X_train=X,
